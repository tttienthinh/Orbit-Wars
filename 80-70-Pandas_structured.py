...
def take_action(df, player_id, nb_steps_sim=NB_STEPS_SIM, return_df=False):
    ...
    # Sun-crossing filter (vectorised outside chain)
    _dx = coarse["x"].values - coarse["x_src"].values
    _dy = coarse["y"].values - coarse["y_src"].values
    _l2 = _dx ** 2 + _dy ** 2
    _dot = (CENTER - coarse["x_src"].values) * _dx + (CENTER - coarse["y_src"].values) * _dy
    _t_sun = np.clip(_dot / np.where(_l2 == 0, 1.0, _l2), 0.0, 1.0)
    _proj = np.sqrt(
        (CENTER - coarse["x_src"].values - _t_sun * _dx) ** 2 +
        (CENTER - coarse["y_src"].values - _t_sun * _dy) ** 2
    )
    _sun_dist = np.where(
        _l2 == 0,
        np.sqrt((CENTER - coarse["x_src"].values) ** 2 + (CENTER - coarse["y_src"].values) ** 2),
        _proj,
    )
    _crossing_sun = _sun_dist < (SUN_RADIUS + PLANET_MARGIN)
    ...
    coarse = (
        ...
        .loc[lambda d:
            (d["dist_tgt_src"] <
             (d["step_diff"] + 1) * MAX_SPEED
             + d["radius_src"] + PLANET_MARGIN + d["radius"]
    ...
    # Phase B: fleet-speed-specific filter + prev_pos join
    _fs_ratio = np.clip(
        np.log(expanded["ships_sent"].values.astype(float)) / math.log(1000.0),
        0, None,
    )
    _fleet_speed_b = 1.0 + (MAX_SPEED - 1.0) * _fs_ratio ** 1.5
    _dist_min_b = expanded["step_diff"].values * _fleet_speed_b + PLANET_MARGIN + expanded["radius_src"].values
    _dist_prev_b = _dist_min_b - _fleet_speed_b

    expanded = (
        expanded
        .assign(fleet_speed=_fleet_speed_b, dist_min=_dist_min_b, dist_prev=_dist_prev_b)
        .loc[lambda d: d["dist_tgt_src"] < d["dist_min"] + d["fleet_speed"] + d["radius"] + PLANET_MOVEMENT_SLACK]
        .merge(prev_pos, on=["id", "step"], how="left")
        .reset_index(drop=True)
    )

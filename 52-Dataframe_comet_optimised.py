...
class IntervalProcessor:
    ...
    def merge_intervals(intervals):
        """Highly optimized interval merger using standard list operations."""
        if not intervals:
            return []
    ...
    def create_cumulative_obstacles(possible_attacks, min_step=0):
        """Optimized timeline construction using vectorized grouping and forward accumulation."""
        max_step = int(possible_attacks["step"].max())
        
        # 1. Pre-unwrap intervals completely using vectorized numpy operations
        # This removes the row-by-row .iterrows() loop completely
        amins = possible_attacks["angle_min"].values
        amaxs = possible_attacks["angle_max"].values
        wrapped = amins > amaxs
        
        # Build an efficient map of key -> list of raw intervals
        attack_map = {}
        
        # Zip combinations for extremely fast iteration compared to iterrows
        keys = zip(
        ...
        for i, key in enumerate(keys):
            ...
            if wrapped[i]:
                attack_map[key].append((amin, 2 * np.pi))
                attack_map[key].append((0.0, amax))
            else:
                attack_map[key].append((amin, amax))

        # 2. Extract unique paths efficiently
        unique_paths = possible_attacks[["id_src", "ships_sent"]].drop_duplicates().values
        
        # Pre-allocate dictionary arrays for speed before conversion to DataFrame
        cumulative_records = []
        
        for id_src, ship in unique_paths:
            current_intervals = []
            merged = []
            
            # Continuous timeline simulation loop
            for step in range(min_step, max_step + 1):
                ...
                # Appending dictionaries is fast; creating dataframes inside loops is slow
                cumulative_records.append({
                    "step": step + 1,
                    "id_src": id_src,
                    "ships_sent": ship,
                    "obstacle_list": merged # Retains reference to last successfully merged state
                })
    ...
    def subtract_intervals(target_min, target_max, blocked_intervals):
        """Subtracts blocked intervals from a single window segment."""
        ...
        for b_min, b_max in blocked_intervals:
            ...
            for s_min, s_max in safe_intervals:
                ...
                # Case 2: Overlap cuts into the interval
                else:
                    if b_min > s_min:
    ...
    @staticmethod
    def compute_free_angles_vectorized(df):
        """Replaces df.apply(axis=1) with a fast list comprehension over zipped raw values."""
        amins = df["angle_min"].values
        amaxs = df["angle_max"].values
        obstacles_col = df["obstacle_list"].values
        
        results = []
        TWO_PI = 2 * np.pi
        EPSILON = 1e-9
        
        # Iterating over zipped numpy arrays avoids Pandas series row extraction overhead entirely
        for amin, amax, obstacles in zip(amins, amaxs, obstacles_col):
            ...
            # Handle wrapping splits natively without DataFrame context
            ...
            # Re-wrap adjacent boundary pieces if necessary
            if len(all_free_intervals) > 1:
                has_end_piece = False
                has_start_piece = False
                end_idx = -1
                start_idx = -1
                
                for idx, (f_min, f_max) in enumerate(all_free_intervals):
                    if abs(f_max - TWO_PI) < EPSILON:
                        has_end_piece = True
                        end_idx = idx
                    if abs(f_min - 0.0) < EPSILON:
                        has_start_piece = True
                        start_idx = idx
                
                if has_end_piece and has_start_piece:
                    wrapped_interval = (all_free_intervals[end_idx][0], all_free_intervals[start_idx][1])
                    # Reconstruct list ignoring merged items
                    all_free_intervals = [
                        f for i, f in enumerate(all_free_intervals) if i != end_idx and i != start_idx
                    ]
                    all_free_intervals.append(wrapped_interval)
                    
            results.append(all_free_intervals)
...
def take_action(df, player_id, nb_steps_sim=NB_STEPS_SIM, return_df=False):
    ...
    attacks_with_angle = (
        ...
        .assign(
            angle_list=lambda d: IntervalProcessor.compute_free_angles_vectorized(d)
        )

...
def take_action(df, player_id, nb_steps_sim=NB_STEPS_SIM):
    ...
    expanded_mine = (
        mine_across_sim
        .assign(ships_sent=mine_across_sim["ships_min"].apply(lambda n: list(range(1, n + 1))))
        .explode("ships_sent")
    ...
    possible_attacks = (
        ...
        .assign(
            ...
            dist_fleet_src=lambda d: d["step_diff"] * d["fleet_speed"] + PLANET_MARGIN + d["radius_src"],
            collision=lambda d: (d["dist_tgt_src"] - d["radius_src"] < d["dist_fleet_src"]) & (d["dist_fleet_src"] < d["dist_tgt_src"] + d["radius"]),
        )
        ...
        .assign(
            ...
            radius_angle=lambda d: np.arccos(((d["dist_tgt_src"]**2 + d["dist_fleet_src"]**2 - d["radius"]**2) / (2 * d["dist_tgt_src"] * d["dist_fleet_src"])).clip(-1, 1)),
    ...
    attacks = (
        ...
        # .groupby("id_src", as_index=False)
        # .head(5) # Only attack the first five

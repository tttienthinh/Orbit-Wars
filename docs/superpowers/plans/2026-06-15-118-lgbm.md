# 118-LGBM Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Create `118-LGBM.py` — a LightGBM binary classifier predicting whether a (src, tgt, game_step) pair will be attacked, producing train/test AUC for direct comparison against 117-NewGNN.

**Architecture:** Single-file script. `build_episode_features(ep_dir)` iterates game steps, uses Polars vectorized cross-joins and pivots (no Python inner loops over pairs), and returns a flat DataFrame. `main()` splits 542 episodes by random shuffle (to mix easy/hard), builds the full dataset, trains LightGBM, and logs to MLflow.

**Tech Stack:** polars, lightgbm, mlflow, sklearn (roc_auc_score), tqdm, numpy

**Spec:** `docs/superpowers/specs/2026-06-15-118-lgbm-design.md`

---

### Task 1: Scaffold 118-LGBM.py — imports, constants, FEATURE_COLS
...
- [ ] **Step 2: Verify it parses and feature count is correct**

```bash
python -c "exec(open('118-LGBM.py').read()); print(f'FEATURE_COLS has {len(FEATURE_COLS)} features')"
```

Expected: `FEATURE_COLS has 95 features`

- [ ] **Step 3: Commit**

```bash
git add 118-LGBM.py
git commit -m "feat: scaffold 118-LGBM.py with constants and 95-feature column list"
```

---

### Task 2: Add `_snap_features` helper and pair enumeration

Implement the cross-join (owner-0 src × all tgt planets, drop self-pairs) and the 12-feature snapshot for both src and tgt at game_step t.

**Files:**
- Modify: `118-LGBM.py`

- [ ] **Step 1: Add `_snap_features` after FEATURE_COLS**
...
- [ ] **Step 2: Add `build_episode_features` stub (snapshot pairs only)**
...
Add after `build_episode_features`:
...
```bash
python -c "exec(open('118-LGBM.py').read()); _test_snap_features()"
```

Expected: `_test_snap_features PASSED`

- [ ] **Step 5: Commit**

```bash
git add 118-LGBM.py
git commit -m "feat: add _snap_features helper and pair cross-join in build_episode_features"
```

---

### Task 3: Add travel time features

Pivot `reach_t` from long (one row per ships_sent) to wide (one column per ships_sent value), normalize by NB_STEPS_SIM, left-join onto pairs.

**Files:**
- Modify: `118-LGBM.py`

- [ ] **Step 1: Insert travel time join inside the `for t` loop, before `step_dfs.append`**

Replace the final two lines of the loop (`step_dfs.append(...)`) with:
...
```bash
python -c "exec(open('118-LGBM.py').read()); _test_travel_features()"
```

Expected: `_test_travel_features PASSED  rows=...  travel_1_non_null=...`

- [ ] **Step 4: Commit**

```bash
git add 118-LGBM.py
git commit -m "feat: add travel time features (pivot reach by ships_sent)"
```

---

### Task 4: Add trajectory features — target ships+owner, source ships

For each step t, slice `df_s` to the window [t, t+20), pivot once for all planets, then join onto pairs as `tgt_ships_t{i}`, `tgt_owner_t{i}`, `src_ships_t{i}`.

**Files:**
- Modify: `118-LGBM.py`

- [ ] **Step 1: Add `_trajectory_pivot` helper before `build_episode_features`**
...
def _trajectory_pivot(
    ...
    ships_piv = df_s_window.pivot(
        values="ships_log", index="id", columns="offset", aggregate_function="first"
    )
    ...
    if owner_prefix is not None:
        owner_piv = df_s_window.pivot(
            values="owner", index="id", columns="offset", aggregate_function="first"
        )
        for i in range(NB_STEPS_SIM):
...
- [ ] **Step 2: Insert trajectory join inside the `for t` loop, before `step_dfs.append`**

Insert after the travel time block and before `step_dfs.append(...)`:
...
Add after `_test_travel_features`:
...
```bash
python -c "exec(open('118-LGBM.py').read()); _test_trajectory_features()"
```

Expected: `_test_trajectory_features PASSED  rows=...`

- [ ] **Step 5: Commit**

```bash
git add 118-LGBM.py
git commit -m "feat: add trajectory features (tgt ships+owner, src ships, offsets 0-19)"
```

---

### Task 5: Add labels and verify the full feature matrix

Left-join actions to assign label=1.0 for attacked pairs; fill nulls with 0.0. Verify all 95 FEATURE_COLS are present and labels are binary.

**Files:**
- Modify: `118-LGBM.py`

- [ ] **Step 1: Insert label join inside the `for t` loop, before `step_dfs.append`**

Insert after the trajectory block and before `step_dfs.append(...)`:
...
Add after `_test_trajectory_features`:
...
```bash
python -c "exec(open('118-LGBM.py').read()); _test_labels()"
```

Expected: `_test_labels PASSED  rows=...  positives=...  pos_rate=...`

- [ ] **Step 4: Commit**

```bash
git add 118-LGBM.py
git commit -m "feat: add label join; verify all 95 FEATURE_COLS present"
```

---

### Task 6: Implement `main()` — split, build, train, evaluate, log to MLflow

**Files:**
- Modify: `118-LGBM.py`

- [ ] **Step 1: Add `main()` after the test functions**
...
- [ ] **Step 2: Add `__main__` block at the bottom of the file**
...
- [ ] **Step 3: Smoke-test on 2 episodes before full run**

```bash
python -c "
exec(open('118-LGBM.py').read())
ep_dirs = sorted([d for d in PRECOMPUTE_DIR.iterdir() if d.is_dir()])[:2]
frames = [build_episode_features(d) for d in ep_dirs]
df = __import__('polars').concat(frames)
missing = [c for c in FEATURE_COLS if c not in df.columns]
...
Expected: `OK: ... rows, ... columns` (columns count ≥ 97: 95 features + label + game_step)

- [ ] **Step 4: Commit**

```bash
git add 118-LGBM.py
git commit -m "feat: add main() with train/test split, LightGBM, MLflow logging"
```
...
```
Episodes: 542 total | 433 train | 109 test
Train: ...M pairs | ... pos | pos_weight=...
Test:  ...M pairs  | ... pos

Train AUC: 0.????  |  Test AUC: 0.????

Top 20 features:
  travel_1                       ...
  ...
Done. Model saved to 118-LGBM/model.txt
```
...
**Spec coverage:**
- ✓ All 95 features: 12 src + 12 tgt + 11 travel + 20 tgt_ships + 20 tgt_owner + 20 src_ships
- ✓ `is_moving` boolean (no angular velocity — confirmed with user)
- ✓ Owner one-hot for both src and tgt (5 classes: -1, 0, 1, 2, 3)
- ✓ Travel time = (arrival − t) / NB_STEPS_SIM per REACH_POW2 ship size, NaN if no edge
- ✓ Target trajectory ships+owner for offsets 0..19 (fill 0.0 / -2.0 if missing)
- ✓ Source trajectory ships for offsets 0..19 (fill 0.0 if missing)
- ✓ Random shuffle of episode IDs before split (mixes easy/hard)
- ✓ TRAIN_RATIO = 0.8
- ✓ pos_weight = n_neg / n_pos for class imbalance
- ✓ LGBM_PARAMS = n_estimators=500, lr=0.05, num_leaves=63
- ✓ MLflow experiment "118-LGBM" with params + metrics + model artifact
- ✓ Both train_auc and test_auc logged and printed
- ✓ Model saved to 118-LGBM/model.txt
- ✓ Feature importances printed (top 20)

**Type consistency:**
- `_snap_features(df, id_alias, prefix)` → `pl.DataFrame` with `id_alias` col — used in Task 2 ✓
- `_trajectory_pivot(df_s_window, id_alias, ships_prefix, owner_prefix)` → `pl.DataFrame` with `id_alias` col — used in Task 4 ✓
- `build_episode_features(ep_dir)` → `pl.DataFrame` with FEATURE_COLS + "label" + "game_step" — used in Task 6 ✓
- `FEATURE_COLS` defined in Task 1, referenced in Tasks 5, 6 ✓
- `_LOG1024` defined in Task 1, used in `_snap_features` (Task 2) and trajectory loop (Task 4) ✓

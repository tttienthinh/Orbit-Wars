# Planet + Board Refactor Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Refactor `24-Rules_Target_big_prod.py` into `25-Board_refactor.py` — identical move output, cleaner structure: `Planet` as data+physics object, `Board` owning game state, simulation, and move generation.

**Architecture:** `Planet` stores raw data (immutable after `__init__`) and computed state (populated later by `Board`), exposes three setup methods and physics methods. `Board` is constructed from `obs`, runs the 10-step forward sim, orchestrates the planet setup pipeline, and generates moves. `agent()` becomes a one-liner: `return Board(obs).get_moves()`.
...
|------|--------|---------------|
| `25-Board_refactor.py` | Create | Refactored agent (Planet + Board + agent) |
| `tests/test_board_refactor.py` | Create | Equivalence test: assert moves from 24 == moves from 25 |

---

### Task 1: Write the equivalence test (failing)

**Files:**
- Create: `tests/test_board_refactor.py`
...
```
pytest tests/test_board_refactor.py -v
```

Expected: ERROR — `FileNotFoundError` or `ModuleNotFoundError` because `25-Board_refactor.py` does not exist yet.

---

### Task 2: Create `25-Board_refactor.py` with the Planet class

**Files:**
- Create: `25-Board_refactor.py`

- [ ] **Step 1: Write the full file up to (and including) Planet**
...
Do NOT add Board or agent() yet — save the file as-is after this step.

---

### Task 3: Add Board class to `25-Board_refactor.py`

**Files:**
- Modify: `25-Board_refactor.py` (append Board class after Planet)

- [ ] **Step 1: Append Board class**

Add this directly after the Planet class:
...
- [ ] **Step 2: Append agent() entry point at the bottom of the file**
...
### Task 4: Run equivalence test and commit

**Files:**
- Run: `tests/test_board_refactor.py`

- [ ] **Step 1: Run the equivalence test**

```
pytest tests/test_board_refactor.py::test_board_refactor_equivalence -v -s
```

The `-s` flag lets the `print(obs.remainingOverageTime)` output through (expected — both agents print it).

Expected output:
```
PASSED tests/test_board_refactor.py::test_board_refactor_equivalence
```

If it FAILS with a moves mismatch, compare the printed moves. The most likely causes:
- `compute_neighbors` sort is not ETA-based (check `aiming` is called with `nb_ships=1`)
- `compute_counts` is reading past `[:NB_NEARBY_OUT_OF]` (check slice)
- `_create_actions` condition logic differs from `Planet.create_actions` in `24` (diff them line-by-line)

- [ ] **Step 2: Commit**

```bash
git add 25-Board_refactor.py tests/test_board_refactor.py
git commit -m "refactor: introduce Board class, split Planet setup methods (25-Board_refactor.py)"
```

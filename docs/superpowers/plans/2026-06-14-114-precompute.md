# 114-precompute Update — Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Rewrite `114-precompute.py` to output to `114-precompute/`, remove `attacks.parquet`, expand `reach.parquet` to all pow-2 fleet sizes, and resolve `actions.parquet` target IDs from actual fleet trajectory data in the replay using `swept_pair_hit`.

**Architecture:** Single file modified: `114-precompute.py`. The step loop is restructured to collect raw planet/fleet snapshots (before the status check, so DONE steps are also captured). After all steps are processed, a second pass builds `fleet_life`, matches each action to its spawned fleet, and uses `swept_pair_hit` to determine which planet the fleet hit. `swept_pair_hit` is inlined; `fleet_speed` is reused from the already-imported `113` module.

**Tech Stack:** Python, Polars, tqdm, math (stdlib).

---

### Task 1: Update header — constants, helpers, OUT_ROOT

**Files:**
- Modify: `114-precompute.py`

- [ ] **Step 1: Replace lines 33–47 (REPLAYS_DIR through ATTACK_POW2)**
...
Also update the module docstring (lines 1–16) to reflect the new outputs:
...
- [ ] **Step 2: Verify syntax (no import/parse errors)**

```bash
python -c "
import importlib.util as _ilu, sys
s = _ilu.spec_from_file_location('m', '114-precompute.py')
m = _ilu.module_from_spec(s)
s.loader.exec_module(m)
print('OK — OUT_ROOT:', m.OUT_ROOT)
print('REACH_POW2:', m.REACH_POW2)
"
```

Expected:
```
OK — OUT_ROOT: 114-precompute
REACH_POW2: [1, 2, 4, 8, 16, 32, 64, 128, 256, 512, 1024]
```
...
```bash
git add 114-precompute.py
git commit -m "refactor: update 114-precompute constants, add swept_pair_hit, OUT_ROOT=114-precompute"
```

---

### Task 2: Restructure `_process_episode` — new signature, loop, geometry

**Files:**
- Modify: `114-precompute.py` (lines 63–166)

Replace the entire `_process_episode` function body up through (but not including) the `# ── Batch dedup` block with the following. Key changes vs the original:
- Return type drops `atk_out` (3-tuple instead of 4)
- `atk_parts` removed; `planet_snap`, `fleet_snap`, `raw_actions` added
- `obs_dict` retrieval and `obs` namespace creation move **before** the status check so DONE steps still contribute snapshots
- Geometry pass uses `REACH_POW2` (no sub-filter for reach); attacks block removed
- Action collection replaced by lightweight `raw_actions` list (no target lookup yet)
...
- [ ] **Step 1: Apply the above replacement**
...
s.loader.exec_module(m)
import inspect
print(inspect.signature(m._process_episode))
"
```

Expected:
```
(ep_json: dict, player_idx: int) -> tuple[polars.dataframe.frame.DataFrame, polars.dataframe.frame.DataFrame, polars.dataframe.frame.DataFrame]
```
...
```bash
git add 114-precompute.py
git commit -m "feat: restructure step loop — expand reach to REACH_POW2, collect snapshots, remove attacks"
```

---

### Task 3: Post-loop — fleet_life, target resolution, assembly, return

**Files:**
- Modify: `114-precompute.py` (the `# ── Batch dedup` block through `return`)

Replace everything from `# ── Batch dedup and assemble output ───` (the old post-loop block) through the `return` statement with:
...
```bash
git add 114-precompute.py
git commit -m "feat: two-pass action target resolution via fleet tracking + swept_pair_hit"
```

---

### Task 4: Update `main()` — 3-file output, skip check, unpack

**Files:**
- Modify: `114-precompute.py` (lines 193–237)

Three edits in `main()`:

**Edit A** — `all_files` tuple (remove `attacks.parquet`):
```python
        all_files = ("df_s.parquet", "reach.parquet", "actions.parquet")
```

**Edit B** — unpack and write calls (remove `attacks` variable and its write):
```python
...
```bash
git add 114-precompute.py
git commit -m "feat: update main() for 3-file output, remove attacks.parquet write"
```

---

### Task 5: Smoke test — run on one episode, verify outputs

**Files:** none (verification only)

- [ ] **Step 1: Run on one episode in a Python shell**

```python
import json, importlib.util as _ilu
from pathlib import Path

s = _ilu.spec_from_file_location("m114", "114-precompute.py")
m = _ilu.module_from_spec(s)
s.loader.exec_module(m)

meta = json.loads(Path("110-replays/53402535/_metadata.json").read_text())
meta_by_id = {int(e["id"]): e for e in meta}

for ep_path in sorted(Path("110-replays/53402535").glob("episode_*.json"))[:20]:
    ep_id = int(ep_path.stem.split("_")[1])
    ep_meta = meta_by_id.get(ep_id)
    player_idx = m._winner_idx(ep_meta) if ep_meta else None
    if player_idx is None:
        continue
    ep_json = json.loads(ep_path.read_text())
    df_s, reach, actions = m._process_episode(ep_json, player_idx)
    print("df_s rows:", len(df_s), "| schema:", list(df_s.schema.keys()))
    print("reach rows:", len(reach), "| ships_sent unique:", sorted(reach["ships_sent"].unique().to_list()))
    print("actions rows:", len(actions), "| non-null id:", actions["id"].drop_nulls().len())
    break
```

Expected (approximate values):
```
df_s rows: 3000–8000  | schema: ['id', 'step', 'x', 'y', 'ships', 'owner', 'production', 'nature']
reach rows: 30000–150000 | ships_sent unique: [1, 2, 4, 8, 16, 32, 64, 128, 256, 512, 1024]
actions rows: 100–600   | non-null id: >80% of actions rows
```

If `non-null id` is 0, the fleet matching is broken — check that `fleet_snap` is being populated (add a `print(len(fleet_snap))` after the loop to confirm it's non-empty).

- [ ] **Step 2: Commit (no code change unless a fix was needed)**

```bash
git add 114-precompute.py
git commit -m "fix: smoke-test corrections (if any)"
```

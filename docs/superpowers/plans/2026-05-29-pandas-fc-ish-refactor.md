# Pandas FC/IS Refactor Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Refactor `80-70-Pandas_structured.py` into a Functional Core / Imperative Shell architecture, producing `81-80-Pandas_fc_ish.py`.

**Architecture:** All constants live in `GameConfig`; physics helpers live in `PhysicsEngine`; the Pandas pipeline is split into four `StrategyPipeline` static methods; the top-level `agent()` orchestrates them. Module-level aliases are added after each class so the untouched `interpreter` function can still reference `MAX_SPEED`, `CENTER`, `swept_pair_hit`, etc. by their original names.

**Tech Stack:** Python 3.10+, pandas, numpy, math, copy, collections.namedtuple
...
|------|--------|----------------|
| `81-80-Pandas_fc_ish.py` | Create | Entire refactored agent (single file) |

---

## Task 1: Scaffold — imports, GameConfig, PhysicsEngine

**Files:**
- Create: `81-80-Pandas_fc_ish.py`

- [ ] **Step 1: Create the file with imports, GameConfig, PhysicsEngine, and module-level aliases**
...
```
python -c "exec(open('81-80-Pandas_fc_ish.py').read()); print('scaffold OK')"
```
...
```bash
git add 81-80-Pandas_fc_ish.py
git commit -m "feat: scaffold GameConfig + PhysicsEngine for FC/IS refactor"
```

---

## Task 2: Interpreter block (verbatim copy)

**Files:**
- Modify: `81-80-Pandas_fc_ish.py`

- [ ] **Step 1: Append the interpreter block verbatim from `80-70-Pandas_structured.py` lines 16–273**

Copy lines 16–273 of the source exactly (namedtuples + `interpreter` function + board constants) and append to `81-80-Pandas_fc_ish.py`:
...
> **Note:** The interpreter defines its own `distance`, `point_to_segment_distance`, `swept_pair_hit` at module level — these re-definitions shadow the earlier aliases. That is fine and intentional: the interpreter block is copied verbatim and must remain self-contained.

- [ ] **Step 2: Verify import still works**

```
python -c "exec(open('81-80-Pandas_fc_ish.py').read()); print('interpreter OK')"
```

Expected: `interpreter OK`

- [ ] **Step 3: Commit**

```bash
git add 81-80-Pandas_fc_ish.py
git commit -m "feat: add verbatim interpreter block to FC/IS scaffold"
```

---

## Task 3: StrategyPipeline._01_get_obs_dataframe

**Files:**
- Modify: `81-80-Pandas_fc_ish.py`

- [ ] **Step 1: Append the StrategyPipeline class opening and `_01` method**
...
- [ ] **Step 2: Smoke test `_01` with a minimal mock obs**

```python
# Append temporarily to file, then remove after confirming
from types import SimpleNamespace
_obs = SimpleNamespace(
    planets=[[0, 0, 30.0, 50.0, 3.0, 20, 2], [1, 1, 70.0, 50.0, 3.0, 20, 2]],
    initial_planets=[[0, 0, 30.0, 50.0, 3.0, 20, 2], [1, 1, 70.0, 50.0, 3.0, 20, 2]],
    fleets=[], next_fleet_id=0, comets=[], comet_planet_ids=[], angular_velocity=0.01,
    player=0,
)
df_s, planet_disp = StrategyPipeline._01_get_obs_dataframe(_obs, 0, 2)
assert list(df_s.columns) == ["step", "id", "x", "y", "radius", "ships", "production", "owner", "nature"]
assert "planet_disp" in planet_disp.columns
print("_01 OK", df_s.shape)
```

Run: `python 81-80-Pandas_fc_ish.py`  
Expected: `_01 OK (22, 9)` (11 steps × 2 planets)

- [ ] **Step 3: Remove the smoke test block, commit**

```bash
git add 81-80-Pandas_fc_ish.py
git commit -m "feat: implement StrategyPipeline._01_get_obs_dataframe"
```

---

## Task 4: StrategyPipeline._02_get_all_opportunities

**Files:**
- Modify: `81-80-Pandas_fc_ish.py`

- [ ] **Step 1: Add `_02_get_all_opportunities` as a second static method inside `StrategyPipeline`**
...
- [ ] **Step 2: Smoke test `_02` (appended temporarily)**
...
print("_02 OK, pa shape:", pa.shape)
assert isinstance(pa, pd.DataFrame)
```

Run: `python 81-80-Pandas_fc_ish.py`  
Expected: `_02 OK, pa shape: (N, M)` — no crash, `pa` is a DataFrame.

- [ ] **Step 3: Remove smoke test, commit**

```bash
git add 81-80-Pandas_fc_ish.py
git commit -m "feat: implement StrategyPipeline._02_get_all_opportunities"
```

---

## Task 5: StrategyPipeline._03_filter_collision

**Files:**
- Modify: `81-80-Pandas_fc_ish.py`

- [ ] **Step 1: Add `_03_filter_collision` inside `StrategyPipeline`**
...
- [ ] **Step 2: Smoke test `_03` (append, remove after)**
...
df_s, planet_disp = StrategyPipeline._01_get_obs_dataframe(_obs, 0, 2)
pa = StrategyPipeline._02_get_all_opportunities(df_s, planet_disp, 0)
awa = StrategyPipeline._03_filter_collision(pa)
print("_03 OK, awa shape:", awa.shape)
assert isinstance(awa, pd.DataFrame)
```

Run: `python 81-80-Pandas_fc_ish.py`  
Expected: `_03 OK, awa shape: (N, M)` — no crash.

- [ ] **Step 3: Remove smoke test, commit**

```bash
git add 81-80-Pandas_fc_ish.py
git commit -m "feat: implement StrategyPipeline._03_filter_collision"
```

---

## Task 6: StrategyPipeline._04_score_and_decide

**Files:**
- Modify: `81-80-Pandas_fc_ish.py`

- [ ] **Step 1: Add `_04_score_and_decide` inside `StrategyPipeline`**
...
- [ ] **Step 2: Smoke test `_04` (append, remove after)**
...
pa = StrategyPipeline._02_get_all_opportunities(df_s, planet_disp, 0)
awa = StrategyPipeline._03_filter_collision(pa)
moves = StrategyPipeline._04_score_and_decide(awa, 0)
print("_04 OK, moves:", moves)
assert isinstance(moves, list)
```

Run: `python 81-80-Pandas_fc_ish.py`  
Expected: `_04 OK, moves: [...]` — list (may be empty for this simple board).

- [ ] **Step 3: Remove smoke test, commit**

```bash
git add 81-80-Pandas_fc_ish.py
git commit -m "feat: implement StrategyPipeline._04_score_and_decide"
```

---

## Task 7: Imperative shell + equivalence verification

**Files:**
- Modify: `81-80-Pandas_fc_ish.py`

- [ ] **Step 1: Append the imperative shell**
...
- [ ] **Step 2: Equivalence test (append temporarily, then remove)**

Run both the old `nearest_planet_sniper` from `80-70-Pandas_structured.py` and the new `agent` from `81-80-Pandas_fc_ish.py` on the same obs and compare:

```python
# Paste after agent() — remove before final commit
from types import SimpleNamespace
...
old = _load("80-70-Pandas_structured.py", "old_agent")

def _make_obs(player):
    return SimpleNamespace(
        planets=[[0, player, 30.0, 50.0, 3.0, 20, 2], [1, 1-player, 70.0, 50.0, 3.0, 20, 2]],
        initial_planets=[[0, player, 30.0, 50.0, 3.0, 20, 2], [1, 1-player, 70.0, 50.0, 3.0, 20, 2]],
        fleets=[], next_fleet_id=0, comets=[], comet_planet_ids=[], angular_velocity=0.01,
        player=player,
    )

old.step = 0; old.num_agents = None; old.player_id = None
obs_old = _make_obs(0)
obs_new = _make_obs(0)
moves_old = old.nearest_planet_sniper(obs_old)
moves_new = agent(obs_new)
print("OLD:", moves_old)
print("NEW:", moves_new)
assert moves_old == moves_new, f"Mismatch!\nOLD: {moves_old}\nNEW: {moves_new}"
print("EQUIVALENCE OK")
```

Run: `python 81-80-Pandas_fc_ish.py`  
Expected: `EQUIVALENCE OK` (or both lists empty for this 2-planet board — check with a more complex obs if needed)

- [ ] **Step 3: Remove equivalence test block, final commit**

```bash
git add 81-80-Pandas_fc_ish.py
git commit -m "feat: complete FC/IS refactor — StrategyPipeline + agent() entry point"
```
...
**Spec coverage check:**
- ✅ `GameConfig` — Task 1
- ✅ `PhysicsEngine` with `@staticmethod`s — Task 1
- ✅ Module-level aliases for interpreter compat — Task 1 note
- ✅ Interpreter block verbatim — Task 2
- ✅ `_01_get_obs_dataframe` → `(df_s, planet_disp: DataFrame)` — Task 3
- ✅ `_02_get_all_opportunities` (mine analysis + cross-join + filters + swept-pair + angles) — Task 4
- ✅ `_03_filter_collision` (blocking self-join) — Task 5
- ✅ `_04_score_and_decide` (comet evasion + scoring + moves list) — Task 6
- ✅ Imperative shell with exact signature — Task 7
- ✅ Single file constraint — all tasks write to one file

**No placeholders found.**

**Type consistency:** `planet_disp` is `pd.DataFrame` in `_01` return and `_02` parameter — consistent throughout. `pa` is `pd.DataFrame` passed from `_02` → `_03` → implicit in `_04`. `moves` is `list` from `_04`.

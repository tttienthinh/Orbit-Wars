...
class StrategyPipeline:
    ...
    def _04_score_and_decide(attacks_with_angle: pd.DataFrame, player_id: int) -> list:
            ...
            if not _c_1_or_2.empty:
                attacks_conqueror_2 = (
                    ...
                    .loc[lambda d:
                        ...
                        (d["ships_sent"] + d["ships_sent_2"] <= np.maximum(d["ships_needed"], d["ships_needed_2"]) + 1)
                    ]
                    .loc[lambda d: d["ships_sent"] <= d["ships_min"]]
                    .sort_values(["step_2", "ships_sent_2"])
                    .groupby(["id_src", "id"], sort=False).first().reset_index()
                    .sort_values(["step_2", "ships_sent_2"])
                    .pipe(lambda d: d if not d.empty else None)
                )
        ...
        # ── Combine and emit ─────────────────────────────────────────────────────
        parts = [df for df in [attacks_conqueror, attacks_conqueror_2, attacks_supplier] if not df.empty]
        if not parts:
            return moves

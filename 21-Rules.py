...
class Planet:
    ...
    def compute_nearby_planets(self, planets_no_comets):
        self.nearby_planets = sorted(
            (p for p in planets_no_comets if p.id != self.id),
    ...
    def create_actions(self) -> list:
        ...
        if self.is_comet:
            ...
            for target in self.nearby_planets[:self.NB_NEARBY_OUT_OF]:
                if target.owner == -1 and self.ships > target.ships * 2:
                    action = self.create_one_action(target, self.ships // 2)
                    if action != []:
                        actions += action
                        return actions  
            for target in self.nearby_planets[:self.NB_NEARBY_OUT_OF]:
                if target.owner != self.owner and self.ships > target.ships * 2:
                    action = self.create_one_action(target, self.ships // 2)
                    if action != []:
                        actions += action

"""Training loop for OrbitGNN (Polars pipeline) — behavioral cloning from _04_get_selected.

Usage:
    python 109-Train_GNN_Polars.py

Requires:
    pip install mlflow kaggle-environments torch torch-geometric polars
"""
import copy
import json
import random
...
# ── Load pipeline from 108 ────────────────────────────────────────────────────
_spec = _ilu.spec_from_file_location("m108", "108-Simulate20Next_GNN_Polars.py")
...
def _process_step(obs_dict: dict) -> "tuple | None":
    ...
    pa_df = SP._02_get_all_opportunities(df_s, planet_disp, 0).collect()
    safe_df = SP._03_filter_collision(pa_df.lazy()).collect()

    if safe_df.is_empty() or "ships_sent" not in safe_df.columns:
        return None
    attack_df = safe_df.filter(pl.col("ships_sent") <= pl.col("ships_min"))
    if attack_df.is_empty():
        return None

    data = SP._05_get_GNN(df_s, pa_df, safe_df)
    if "attack" not in data.node_types or data["attack"].x.numel() == 0:
        return None

    selected = SP._04_get_selected(safe_df.lazy(), player_id=0)
    key_cols = ["id_src", "id", "step", "ships_sent"]
    selected_keys = set(selected.select(key_cols).rows())
    y = torch.tensor(
        [1.0 if row in selected_keys else 0.0
         for row in attack_df.select(key_cols).rows()],
        dtype=torch.float32,
    )
    return data, y
...
def main() -> None:
    ...
    agent_108_path = Path("108-Simulate20Next_GNN_Polars.py")
    ...
    with mlflow.start_run():
        ...
        while True:
            ...
            try:
                replay = run_game(agent_108_path, opponent_path)
            except Exception as e:
            ...
            print(f"Game {game_idx} vs {opponent_path.name}: {'WIN' if winner_is_gnn else 'LOSS'}")

...
class StrategyPipeline:
    ...
    def _02_get_reach(
        ...
        # All planets as sources — no owner filter
        all_base_lf = (
            df_s_lf
            .group_by("id", maintain_order=True)
            .agg([
        ...
        coarse_lf = (
            ...
            # Fixed discrete ships_sent values instead of int_ranges
            .with_columns(
                pl.lit(ships_list).alias("ships_sent")
            )
            .explode("ships_sent")
        )
    ...
    @staticmethod
    def _04_get_selected(safe_lf: pl.LazyFrame, player_id: int) -> pl.DataFrame:
        attacks_with_angle = safe_lf.collect()
        if attacks_with_angle.is_empty():
            return attacks_with_angle.select(["id_src", "id", "step", "ships_sent"])
        ...
        if not awa_comets.is_empty():
            ...
            if max(x_off, y_off) > 45:
                ...
                if not comet_sel.is_empty():
                    selected_parts.append(comet_sel.select(["id_src", "id", "step", "ships_sent"]))
                id_to_avoid = awa_comets["id_src"].unique().to_list()
                attacks_with_angle = attacks_with_angle.filter(~pl.col("id_src").is_in(id_to_avoid))

        if attacks_with_angle.is_empty():
            if selected_parts:
                return pl.concat(selected_parts, how="diagonal")
            return attacks_with_angle.select(["id_src", "id", "step", "ships_sent"])
        ...
        attacks_joined = (
            planet_id_top_5
            .join(attacks_with_angle, on=["id_src", "id"], how="inner")
        ...
        if attacks_joined.is_empty():
            if selected_parts:
                return pl.concat(selected_parts, how="diagonal")
            return attacks_with_angle.select(["id_src", "id", "step", "ships_sent"]).clear()
        ...
        if not attacks.is_empty():
            selected_parts.append(attacks.select(["id_src", "id", "step", "ships_sent"]))
    ...
    def _05_get_GNN(
        ...
        # ── (id, step) → row index lookup ─────────────────────────────────────
        ps_idx = ps_df.with_row_index("ps_idx").select(["id", "step", "ps_idx"])

        data = HeteroData()
        data["planet"].x = planet_x
        data["planet_step"].x = planet_step_x
        data["planet", "has_snapshot", "planet_step"].edge_index = torch.stack([src_snap, dst_snap])

        # ── reaches edges (all-planets, SHIPS_LIST) ───────────────────────────
        if not reach_df.is_empty():
            r_df = (
                reach_df.select(["id_src", "step_src", "id", "step", "ships_sent"])
                .join(
            ...
            if not r_df.is_empty():
                ships_arr = r_df["ships_sent"].to_numpy().astype(float)
                data["planet_step", "reaches", "planet_step"].edge_index = torch.tensor(
                    np.stack([r_df["src_r"].to_numpy(), r_df["dst_r"].to_numpy()]),
                    dtype=torch.long,
                )
...
class OrbitGNN(nn.Module):
    ...
    def forward(self, data) -> torch.Tensor:
        has_attack = "attack" in data.node_types and data["attack"].x.numel() > 0

"""
Debug8: Reproduce exactly the same code path as test6_lazy.py at step 64,
including using the EXACT same functions from the original script.
Print intermediate results at each step of the attacks pipeline for id_src=20.
"""
...
# ─── Exact copy of take_action_lazy from test6_lazy.py, with debug prints ─────
def take_action_lazy_debug(df, player_id, nb_steps_sim=NB_STEPS_SIM):
    ...
    if mine_across_sim.is_empty(): return []
    ...
    if possible_attacks.is_empty(): return []
    ...
    if not awa_comets.is_empty():
        ...
        if max(x_off,y_off)>45:
            comet_rows = (
                awa_comets.filter(pl.col("ships_sent")<=pl.col("ships_min"))
    ...
    # DEBUG: show top-5 for id_src=20
    top5_lz = (
        attacks_with_angle.lazy().sort(["step","ships_sent"])
    ...
    top5_20 = top5_lz.filter(pl.col("id_src")==20)
    print(f"  [LZ debug] top-5 for id_src=20: ids={sorted(top5_20['id'].to_list())}")
    ...
    # DEBUG: show attacks for id_src=20
    atk_20 = attacks.filter(pl.col("id_src")==20)
    print(f"  [LZ debug] attacks for id_src=20: {atk_20.select(['id_src','id','ships_sent','step','ships_min','score']).to_pandas().to_string()}")
...
# We need to track actual moves for proper game state
# Use a simple stub that replays the same pandas logic
...
def take_action(df, player_id, nb_steps_sim=NB_STEPS_SIM):
    ...
    if pa.empty: return []
    df_obs = IntervalProcessor.create_cumulative_obstacles(pa)
    awa = (
        pa.merge(df_obs,how="left",on=["id_src","step","ships_sent"])
        .assign(angle_list=lambda d:d.apply(IntervalProcessor.compute_free_angles,axis=1))
        .query("angle_list.str.len() > 0")
    )
    top5 = (
        awa.sort_values(["step","ships_sent"],ascending=True)
    ...
    awa_comets=awa.query("nature_src=='comet'");moves=[]
    if not awa_comets.empty and max(max((awa_comets["x_src"]-CENTER).abs()),max((awa_comets["y_src"]-CENTER).abs()))>45:
        moves+=(awa_comets.query("ships_sent<=ships_min").sort_values(["ships_sent","step"],ascending=[False,True])
    ...
    attacks=(top5.merge(awa,how="left",on=["id_src","id"]).query("@player_id != owner")
...
for env_step in range(N_STEPS):
    ...
    moves_pd = take_action(df, player_id=0)

    if env_step == 64:
        print(f"=== Step 64 ===")
        print(f"PD moves: {moves_pd}")
        moves_lz_debug = take_action_lazy_debug(df, player_id=0)
        print(f"LZ moves: {moves_lz_debug}")
        break

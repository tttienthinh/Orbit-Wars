...
class StrategyPipeline:
    ...
    def _04_minimax_search(safe_lf: pl.LazyFrame, obs, board: "Board") -> list:
        ...
        # ── BFS Phase 2 (full-slice): stack complete P-planet slices ─────
        slices = []
        for c0_id, df_ck in c0_variants:
            s = (
                df_ck
                .filter(pl.col("step") >= step5_from)
                .with_columns(pl.lit(c0_id).alias("c0_id"))
            )
            slices.append(s)
        df_ships_batched = pl.concat(slices)

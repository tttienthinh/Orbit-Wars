...
OUT_DIR        = Path("119-NewGNN_autoresearch")
NB_STEPS_SIM   = 20
N_EPOCHS           = 200
...
def main() -> None:
    ...
    checkpoints = sorted(
        OUT_DIR.glob("model_epoch*.pt"),
        key=lambda p: int(p.stem.replace("model_epoch", ""))
    )
    if checkpoints:
        latest_ckpt = checkpoints[-1]
        start_epoch = int(latest_ckpt.stem.replace("model_epoch", "")) + 1
        model.load_state_dict(torch.load(latest_ckpt, map_location="cpu"))
        _log(f"Resumed from {latest_ckpt} (starting at epoch {start_epoch})", log_fh)
    else:
        start_epoch = 1
        _log("No checkpoint found, starting from scratch", log_fh)

    mlflow.set_experiment("119-NewGNN_autoresearch")

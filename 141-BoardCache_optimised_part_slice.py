...
class StrategyPipeline:
    ...
    @staticmethod
    def _02_get_all_opportunities_batched(
        df_s: pl.DataFrame,
        planet_disp: pl.DataFrame,
        pos_df: pl.DataFrame,
        player_id: int,
    ) -> pl.LazyFrame:
        """Batched version of _02: df_s has a c0_id column; one Polars call for all c0 variants."""
        df_s_lf = df_s.lazy()
        planet_disp_lf = planet_disp.lazy()
        ...
        mine_base_lf = (
            ...
            .group_by(["c0_id", "id"], maintain_order=True)
            .agg([
                pl.first("step").alias("step_src"),
                pl.first("x").alias("x_src"),
                pl.first("y").alias("y_src"),
        ...
        # inner join on c0_id = cross-product within each c0_id partition
        coarse_lf = (
            mine_base_lf
            .join(df_s_lf, on="c0_id", how="inner")
            .filter(
                (pl.col("step") > pl.col("step_src")) & (pl.col("id") != pl.col("id_src"))
            )
            .join(planet_disp_lf, on=["id", "step"], how="left")
        ...
        # Use pos_df (no c0_id) to avoid multiplying prev_pos rows across c0_ids
        prev_pos_lf = (
            pos_df.lazy()
            .select(["id", "step", "x", "y"])
            .rename({"x": "x_prev", "y": "y_prev"})
    ...
    @staticmethod
    def _03_filter_collision_batched(pa_lf: pl.LazyFrame) -> pl.LazyFrame:
        """Batched version of _03: blocking is computed within each c0_id partition."""
        angle_norm = pl.col("angle") % (2 * math.pi)
        wraps = pl.col("angle_min_obs") > pl.col("angle_max_obs")
        in_cone = pl.when(wraps).then(
            (angle_norm >= pl.col("angle_min_obs")) | (angle_norm <= pl.col("angle_max_obs"))
        ...
        blocked_lf = (
            pa_lf.select(["c0_id", "id_src", "ships_sent", "step", "id", "angle", "angle_min", "angle_max"])
            .join(
                pa_lf.select(["c0_id", "id_src", "ships_sent", "step", "id", "angle_min", "angle_max"])
                     .rename({"step": "step_obs", "id": "id_obs",
                              "angle_min": "angle_min_obs", "angle_max": "angle_max_obs"}),
                on=["c0_id", "id_src", "ships_sent"],
                how="inner",
            )
            .filter(
                (pl.col("step_obs") < pl.col("step")) & (pl.col("id_obs") != pl.col("id"))
            ...
            .filter(in_cone)
            .select(["c0_id", "id_src", "ships_sent", "step", "id"])
            .unique()
        )

        return (
            pa_lf
            .join(blocked_lf, on=["c0_id", "id_src", "ships_sent", "step", "id"], how="anti")
            .with_columns(pl.col("angle").alias("final_angle"))
        )
    ...
    def _04_minimax_search(safe_lf: pl.LazyFrame, obs, board: "Board") -> list:
        ...
        # ── Comet evasion (unchanged from 138) ───────────────────────────
        ...
        # ── BFS Phase 1: build all per-c0 ship states (pure Python) ──────
        c0_variants: list = [(0, df_ships_full)]  # (c0_id, df_ships)
        for k, (c0_move, c0_tgt_id, c0_step_tgt) in enumerate(step0_candidates[1:], 1):
            df_ck = board._recompute_from_sim(df_ships_full, c0_move, c0_tgt_id, c0_step_tgt)
            c0_variants.append((k, df_ck))

        # ── BFS Phase 2 (partial-slice): stack only dirty planets per c0_id ─
        baseline_slice = (
            df_ships_full
            .filter(pl.col("step") >= step5_from)
            .with_columns(pl.lit(0).alias("c0_id"))
        )
        expand_slices = [baseline_slice]
        for k, (c0_move, c0_tgt_id, _) in enumerate(step0_candidates[1:], 1):
            _, df_ck = c0_variants[k]
            dirty_pids = {c0_move[0]}
            if c0_tgt_id is not None:
                dirty_pids.add(c0_tgt_id)
            dirty_list = list(dirty_pids)
            dirty_rows = (
                df_ck
                .filter(pl.col("id").is_in(dirty_list) & (pl.col("step") >= step5_from))
                .with_columns(pl.lit(k).alias("c0_id"))
            )
            clean_rows = (
                baseline_slice
                .filter(~pl.col("id").is_in(dirty_list))
                .with_columns(pl.lit(k).alias("c0_id"))
            )
            expand_slices.append(pl.concat([clean_rows, dirty_rows]))
        df_ships_batched = pl.concat(expand_slices)

        # ── BFS Phase 3: one batched _02+_03 call ────────────────────────
        df_s5, planet_disp5, pos5 = board.build_df_s_slice_batched(df_ships_batched, step5_from)
        pa5_batched  = StrategyPipeline._02_get_all_opportunities_batched(
            df_s5, planet_disp5, pos5, player_id
        )
        safe5_all = StrategyPipeline._03_filter_collision_batched(pa5_batched).collect()

        # ── BFS Phase 4: split results by c0_id ──────────────────────────
        safe5_by_c0: dict = {}
        for c0_id, _ in c0_variants:
            if safe5_all.is_empty():
                safe5_by_c0[c0_id] = safe5_all
            else:
                safe5_by_c0[c0_id] = safe5_all.filter(pl.col("c0_id") == c0_id)
        safe5_base = safe5_by_c0[0]
        ...
        best_score_base = top3_scored[0][0] if top3_scored else None
        best_c5_base    = top3_scored[0][1] if top3_scored else None
        best_score      = best_score_base
        best_c0         = None

        # ── Per c0: restricted set + evaluate (no Polars, no _02/_03) ────
        for k, (c0_move, c0_tgt_id, _) in enumerate(step0_candidates[1:], 1):
            _, df_ships_c0 = c0_variants[k]
            safe5_c        = safe5_by_c0[k]
            ...
            if not safe5_c.is_empty():
                for filt in ("id_src", "id"):
                    sub = safe5_c.filter(pl.col(filt).is_in(list(covered_srcs)))

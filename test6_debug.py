...
def interpreter(obs, actions, step, num_agents=2):
    ...
    return {
        "planets": obs0.planets, "initial_planets": obs0.initial_planets,
        "fleets": obs0.fleets, "next_fleet_id": obs0.next_fleet_id,
        "comets": obs0.comets, "comet_planet_ids": obs0.comet_planet_ids,
    }
    ...
    def create_cumulative_obstacles(possible_attacks, min_step=0):
        ...
        for id_src, ship in unique_combinations:
            ...
            for step in range(min_step, max_step + 1):
                ...
                cumulative_records.append({"step": step + 1, "id_src": id_src,
                                           "ships_sent": ship, "obstacle_list": merged})
        ...
        .assign(
            crossing_sun=lambda d: d.apply(
                lambda row: point_to_segment_distance(
                    (CENTER, CENTER), (row["x_src"], row["y_src"]), (row["x"], row["y"]),
                ) < SUN_RADIUS + PLANET_MARGIN, axis=1,
            ).astype(bool)
        )
...
def take_action_lazy(df: pd.DataFrame, player_id: int,
    ...
    mine_across_sim = (
        ...
        .agg(
            pl.first("step").alias("step_src"), pl.first("x").alias("x_src"),
            pl.first("y").alias("y_src"), pl.first("radius").alias("radius_src"),
            pl.min("ships").alias("ships_min"), pl.first("production").alias("production_src"),
            pl.first("nature").alias("nature_src"), pl.first("owner").alias("owner_src"),
            pl.len().alias("row_count"), pl.sum("is_mine").alias("is_mine"),
        )
        .filter(
    ...
    attacks = (
        ...
        .with_columns(
            pl.when(pl.col("owner") == -1).then(pl.col("ships")).otherwise(pl.col("ships") + pl.col("production")).alias("ships_needed")
        )
...
for env_step in range(N_STEPS):
    ...
    if len(pd_s) != len(pl_s):
        print(f"\n=== MISMATCH at step {env_step} ===")
        print(f"pandas moves ({len(pd_s)}): {pd_s}")
        print(f"lazy   moves ({len(pl_s)}): {pl_s}")

        # More detailed: run both with return_df=True
        moves_pd_full, pa_pd = take_action(df, player_id=0, nb_steps_sim=NB_STEPS_SIM, return_df=True)
        print("\n--- pandas possible_attacks columns:", pa_pd.columns.tolist())
        print("--- pandas possible_attacks unique (id_src, id):", pa_pd[["id_src", "id"]].drop_duplicates().values.tolist())

        # For lazy, look at intermediate stages manually
        df_lf = pl.from_pandas(df).sort("step").lazy()
        mine_across_sim = (
            df_lf
            .with_columns(pl.when(pl.col("owner") == 0).then(1).otherwise(0).alias("is_mine"))
            .group_by("id", maintain_order=True)
            .agg(
                pl.first("step").alias("step_src"), pl.first("x").alias("x_src"),
                pl.first("y").alias("y_src"), pl.first("radius").alias("radius_src"),
        ...
        print(f"\n--- mine_across_sim pandas:\n{mine_across_sim.to_pandas()[['id_src','ships_min','production_src','nature_src']].to_string()}")

        # Inspect what pandas mine_across_sim contains
        pd_mine = (
            df
            .assign(is_mine=lambda d: (d["owner"] == 0).astype(int))
            .groupby("id")
            .agg(
                step_src=("step","first"), x_src=("x","first"), y_src=("y","first"),
                radius_src=("radius","first"), ships_min=("ships","min"),
        ...
        print(f"\n--- mine_across_sim polars:\n{pd_mine[['id_src','ships_min','production_src','nature_src']].to_string()}")
        break
    else:
        if env_step % 10 == 0:
            print(f"step {env_step}: OK")

    rng_action = random.choice(moves_lz) if moves_lz else []
    env.step([moves_pd, rng_action if isinstance(rng_action, list) and len(rng_action) == 3 else []])

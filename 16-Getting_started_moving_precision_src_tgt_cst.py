...
# kaggle competitions submit orbit-wars -f 05-Getting_started_moving_precision.py -m "05-Getting_started_moving_precision.py"
...
def angle_to_position(angle, orbital_radius=1):
    x = math.cos(angle) * orbital_radius + CENTER_X
    y = math.sin(angle) * orbital_radius + CENTER_Y
    return x, y
...
def get_first_t(x0, y0, x1, y1, angular_velocity, speed, t_max=20, radius_src=0, radius_tgt=0):
    for t in range(t_max):
        if distance_at_t(x0, y0, x1, y1, angular_velocity, t=t) - t * speed < radius_src + radius_tgt + PLANET_MARGIN:
            return True, t
    return False, t_max
...
def nearest_planet_sniper(obs):
    ...
    for mine in my_planets:
        ...
        for t in targets:
            ...
            if dist < min_dist and t.id not in comet_planet_ids:
                min_dist = dist
                nearest = t
        ...
        if mine.ships >= ships_needed:
            if not is_planet_moving(nearest):
                # Calculate angle from our planet to the target
                angle = math.atan2(nearest.y - mine.y, nearest.x - mine.x)
                moves.append([mine.id, angle, ships_needed])
            else:
                speed = fleet_speed(ships_needed)
                ok, t = get_first_t(mine.x, mine.y, nearest.x, nearest.y, angular_velocity, speed, t_max=100, radius_src=mine.radius, radius_tgt=nearest.radius)
                if ok:
                    new_x1, new_y1 = next_position(nearest.x, nearest.y, angular_velocity, t)
                    print(f"From {mine.id} To {nearest.id} with time {t} at {new_x1}, {new_y1}")
                    angle = math.atan2(new_y1 - mine.y, new_x1 - mine.x)
                    moves.append([mine.id, angle, ships_needed])

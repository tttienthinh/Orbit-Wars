"""
Minimal reproducer for step 64 mismatch.
Run take_action vs take_action_lazy on the same df at step 64
and print the final 'attacks' DataFrame from both to find where they diverge.
"""
...
def interpreter(obs, actions, step, num_agents=2):
    ...
    if expired_comet_pids:
        ...
        for group in obs0.comets: group["planet_ids"] = [pid for pid in group["planet_ids"] if pid not in expired_set]
    ...
    for planet in obs0.planets:
        if planet[1] != -1: planet[5] += planet[6]
    max_speed = MAX_SPEED; fleets_to_remove = []; combat_lists = {p[0]: [] for p in obs0.planets}
    for fleet in obs0.fleets:
        angle = fleet[4]; ships = fleet[6]
        speed = min(1.0+(max_speed-1.0)*(math.log(ships)/math.log(1000))**1.5, max_speed)
        old_pos = (fleet[2], fleet[3]); fleet[2] += math.cos(angle)*speed; fleet[3] += math.sin(angle)*speed
        new_pos = (fleet[2], fleet[3]); hit_planet = False
        for planet in obs0.planets:
        ...
        if not (0<=fleet[2]<=BOARD_SIZE and 0<=fleet[3]<=BOARD_SIZE): fleets_to_remove.append(fleet); continue
        if point_to_segment_distance((CENTER,CENTER), old_pos, new_pos) < SUN_RADIUS: fleets_to_remove.append(fleet); continue
    angular_velocity = obs0.angular_velocity; comet_pid_set = set(obs0.comet_planet_ids)
    initial_by_id = {p[0]: p for p in obs0.initial_planets}
    def sweep_fleets(planet, old_pos, new_pos):
        if old_pos == new_pos: return
        for fleet in obs0.fleets:
            if fleet not in fleets_to_remove and point_to_segment_distance((fleet[2],fleet[3]), old_pos, new_pos) < planet[4]:
    ...
    for planet in obs0.planets:
        ...
        dx = initial_p[2]-CENTER; dy = initial_p[3]-CENTER; r = math.sqrt(dx**2+dy**2)
    ...
    for pid, planet_fleets in combat_lists.items():
        ...
        player_ships = {}
        for fleet in planet_fleets: player_ships[fleet[1]] = player_ships.get(fleet[1], 0)+fleet[6]
        ...
        else: survivor_owner = top_player; survivor_ships = top_ships
        if survivor_ships > 0:
...
def _simulate(obs, global_step, num_agents, n_steps=NB_STEPS_SIM):
    sim = copy.deepcopy(obs); no_actions = [[] for _ in range(num_agents)]; rows = []
    for i in range(n_steps+1):
        for p in sim.planets:
            ...
            rows.append({"step": global_step+i, "id": pid, "x": x, "y": y, "radius": radius,
                         "ships": ships, "production": production, "owner": owner, "nature": nature})
        interpreter(sim, no_actions, global_step+i, num_agents)
    return pd.DataFrame(rows)

# ── Both take_action implementations inlined ──────────────────────────────────
class IntervalProcessor:
    ...
    def compute_free_angles(row):
        ...
        has_end = any(abs(f[1]-2*np.pi)<1e-9 for f in all_free); has_start = any(abs(f[0]-0.0)<1e-9 for f in all_free)
        if has_end and has_start and len(all_free)>1:
            ...
            all_free = [f for i,f in enumerate(all_free) if i not in {end_idx,start_idx}]; all_free.append(wrapped)
        return all_free
...
def take_action_full(df, player_id, nb_steps_sim=NB_STEPS_SIM):
    """Returns (moves, mine_across_sim, possible_attacks, attacks_with_angle, attacks_df)"""
    mine_across_sim = (
    ...
    if possible_attacks.empty: return [], mine_across_sim, possible_attacks, pd.DataFrame(), pd.DataFrame()
    ...
    planet_id_top_5 = (
        attacks_with_angle.sort_values(["step","ships_sent"],ascending=True).groupby(["id_src","id"],as_index=False).first()
        .sort_values(["step","ships_sent"],ascending=True).groupby("id_src",as_index=False).head(5)[["id_src","id"]]
    )
    awa_comets = attacks_with_angle.query("nature_src == 'comet'"); moves = []
    if not awa_comets.empty and max(max((awa_comets["x_src"]-CENTER).abs()),max((awa_comets["y_src"]-CENTER).abs()))>45:
        moves += (awa_comets.query("ships_sent <= ships_min").sort_values(["ships_sent","step"],ascending=[False,True]).groupby("id_src",as_index=False).first()[["id_src","angle","ships_sent"]].values.tolist())
        id_to_avoid = awa_comets["id_src"].unique().tolist(); attacks_with_angle = attacks_with_angle.query("id_src not in @id_to_avoid")
    attacks = (
        planet_id_top_5.merge(attacks_with_angle,how="left",on=["id_src","id"]).query("@player_id != owner")
    ...
    return moves, mine_across_sim, possible_attacks, attacks_with_angle, attacks
...
def take_action_lazy_full(df, player_id, nb_steps_sim=NB_STEPS_SIM):
    """Returns (moves, mine_across_sim, possible_attacks, attacks_with_angle, attacks_df)"""
    ...
    if mine_across_sim.is_empty(): return [], mine_across_sim.to_pandas(), pl.DataFrame().to_pandas(), pd.DataFrame(), pd.DataFrame()
    ...
    possible_attacks = (
        ...
        .filter(pl.col("collision")).with_columns(crossing_sun_expr.alias("crossing_sun")).filter(~pl.col("crossing_sun"))
        ...
        .with_columns([((pl.col("angle")-pl.col("radius_angle"))%(2*math.pi)).alias("angle_min"),
                       ((pl.col("angle")+pl.col("radius_angle"))%(2*math.pi)).alias("angle_max")])
        .sort("step").collect()
    )
    if possible_attacks.is_empty(): return [], mine_across_sim.to_pandas(), possible_attacks.to_pandas(), pd.DataFrame(), pd.DataFrame()

    df_obstacles = IntervalProcessorPolars.create_cumulative_obstacles(possible_attacks)
    attacks_with_angle = (
        possible_attacks.lazy().join(df_obstacles.lazy(),on=["id_src","step","ships_sent"],how="left")
    ...
    awa_comets = attacks_with_angle.filter(pl.col("nature_src")=="comet"); moves = []
    if not awa_comets.is_empty():
        ...
        if max(x_off,y_off)>45:
            comet_rows = (awa_comets.filter(pl.col("ships_sent")<=pl.col("ships_min"))
                          .sort(["ships_sent","step"],descending=[True,False]).group_by("id_src",maintain_order=True).first()
    ...
    attacks = (
        attacks_with_angle.lazy().sort(["step","ships_sent"]).group_by(["id_src","id"],maintain_order=True).first()
        .sort(["step","ships_sent"]).group_by("id_src",maintain_order=True).head(5).select(["id_src","id"])
        .join(attacks_with_angle.lazy(),on=["id_src","id"],how="left").filter(pl.col("owner")!=player_id)
        ...
        .sort(["step","ships_sent"]).group_by(["id_src","id"],maintain_order=True).first()
        ...
        .sort("score",descending=True).group_by("id_src",maintain_order=True).first()
    ...
    return moves, mine_across_sim.to_pandas(), possible_attacks.to_pandas(), attacks_with_angle.to_pandas(), attacks.to_pandas()
...
def random_agent_fn(obs):
    ...
    planet = random.choice(my_planets); ships = planet[5]//2
    if ships < 1: return []
...
for env_step in range(N_STEPS):
    ...
    moves_pd, mine_pd, pa_pd, awa_pd, att_pd = take_action_full(df, player_id=0)
    moves_lz, mine_lz, pa_lz, awa_lz, att_lz = take_action_lazy_full(df, player_id=0)

    key = lambda m: (int(m[0]), int(m[2]))
    pd_s = sorted(moves_pd, key=key); pl_s = sorted(moves_lz, key=key)
    match = len(pd_s)==len(pl_s) and all(
        int(a[0])==int(b[0]) and int(a[2])==int(b[2]) and abs(float(a[1])-float(b[1]))<1e-6
    ...
    if not match:
        ...
        print(f"\n--- mine_across_sim pandas ({len(mine_pd)} rows):")
        print(mine_pd[["id_src","ships_min","production_src","nature_src","step_src"]].to_string())
        print(f"\n--- mine_across_sim lazy ({len(mine_lz)} rows):")
        print(mine_lz[["id_src","ships_min","production_src","nature_src","step_src"]].to_string())

        print(f"\n--- possible_attacks pd unique (id_src,id) ({len(pa_pd)} rows total):")
        print(pa_pd[["id_src","id"]].drop_duplicates().to_string())
        print(f"\n--- possible_attacks lazy unique (id_src,id) ({len(pa_lz)} rows total):")
        print(pa_lz[["id_src","id"]].drop_duplicates().to_string())

        print(f"\n--- attacks_with_angle pd unique (id_src,id) ({len(awa_pd)} rows total):")
        if not awa_pd.empty: print(awa_pd[["id_src","id","ships_sent","step","owner"]].drop_duplicates().sort_values(["id_src","id","step","ships_sent"]).to_string())
        print(f"\n--- attacks_with_angle lazy unique (id_src,id) ({len(awa_lz)} rows total):")
        if not awa_lz.empty: print(awa_lz[["id_src","id","ships_sent","step","owner"]].drop_duplicates().sort_values(["id_src","id","step","ships_sent"]).to_string())

        print(f"\n--- final attacks pd ({len(att_pd)} rows):")
        if not att_pd.empty: print(att_pd[["id_src","id","ships_sent","step","ships_min","ships_needed","score","final_angle"]].to_string())
        print(f"\n--- final attacks lazy ({len(att_lz)} rows):")
        if not att_lz.empty: print(att_lz[["id_src","id","ships_sent","step","ships_min","ships_needed","score","final_angle"]].to_string())

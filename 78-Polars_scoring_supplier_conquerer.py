...
def take_action(df, player_id, nb_steps_sim=NB_STEPS_SIM, return_df=False):
    ...
    mine_planet_ids = mine_base_lf.select(pl.col("id_src").alias("id")).collect()
    top5_ids_nature = (
        top5_ids
        .join(mine_planet_ids.with_columns(pl.lit(True).alias("target_is_mine")), on="id", how="left")
        .with_columns(pl.col("target_is_mine").fill_null(False))
        .group_by("id_src")
        .agg(
        ...
        .select(["id_src", "status"])
    )

    conqueror_src_ids = top5_ids_nature.filter(pl.col("status") == "Conqueror").select("id_src")
    supplier_src_ids = top5_ids_nature.filter(pl.col("status") == "Supplier").select("id_src")
    target_supplier_ids = (
        top5_ids_nature
        .filter(pl.col("status") == "Supplier")
        .select(pl.col("id_src").alias("id"))
        .with_columns(pl.lit(True).alias("target_is_supplier"))
    )

    attacks_conqueror = (
        attacks_with_angle
        .join(conqueror_src_ids, on="id_src", how="inner")
        .with_columns(
    ...
    attacks_supplier = (
        attacks_with_angle
        .join(supplier_src_ids, on="id_src", how="inner")
        .join(top5_ids.select(["id_src", "id", "is_top5"]), on=["id_src", "id"], how="left")
        .with_columns(pl.col("is_top5").fill_null(False))
        .filter(pl.col("is_top5"))
        .join(target_supplier_ids, on="id", how="left")
        .with_columns(pl.col("target_is_supplier").fill_null(False))
        .with_columns(
            pl.when(pl.col("owner") == -1)
            .then(pl.col("ships"))
            .otherwise(pl.col("ships") + pl.col("production"))
        ...
        .sort(["step", "ships_sent"])
        .group_by(["id_src", "id", "step"], maintain_order=True)
        .first()
        .with_columns(
            (
                - pl.when(pl.col("target_is_supplier")).then(pl.lit(100.0)).otherwise(pl.lit(0.0))
                + pl.when(pl.col("nature") == "moving").then(pl.lit(50.0)).otherwise(pl.lit(0.0))
                + pl.col("ships_sent") * (
                    ((pl.col("ships") + pl.col("ships_sent")) * 0.2
                     < (pl.col("ships_min") - pl.col("ships_sent")))
                    & ((pl.col("ships_min") - pl.col("ships_sent"))
                       < (pl.col("ships") + pl.col("ships_sent")) * 0.8)
                ).cast(pl.Float64)
            ).alias("score")
        )
        .filter(pl.col("score") > 0)
    ...
    attacks = pl.concat([attacks_conqueror, attacks_supplier], how="diagonal")

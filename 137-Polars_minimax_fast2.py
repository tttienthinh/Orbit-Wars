...
class StrategyPipeline:
    ...
    def _04_minimax_search(safe_lf: pl.LazyFrame, obs, cache, player_id: int) -> list:
        ...
        # ── Baseline pass: full step-5 evaluation → record top3_c5_base ───
        ...
        for c0_move, c0_tgt_id in step0_candidates[1:]:
            ...
            if not merged5.is_empty():
                filtered_from = merged5.filter(pl.col("id_src").is_in(list(covered_srcs)))
                for row in filtered_from.select(["id_src", "final_angle", "ships_sent"]).iter_rows():
                    restricted.append(list(row))
                filtered_to = merged5.filter(pl.col("id").is_in(list(covered_srcs)))
                for row in filtered_to.select(["id_src", "final_angle", "ships_sent"]).iter_rows():
                    restricted.append(list(row))

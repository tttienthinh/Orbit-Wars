...
def get_first_t(x0, y0, x1, y1, angular_velocity, speed, t_max=20, radius=0):
    for t in range(t_max):
        if distance_at_t(x0, y0, x1, y1, angular_velocity, t=t) - t * speed < radius:
            return True, t
    return False, t_max
    ...
    for mine in my_planets:
        ...
        if mine.ships >= ships_needed:
            if not is_planet_moving(nearest):
                ...
                ok, t = get_first_t(mine.x, mine.y, nearest.x, nearest.y, angular_velocity, speed, t_max=100, radius=mine.radius)
                if ok:

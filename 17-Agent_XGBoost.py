"""
17-Agent_XGBoost.py — Test XGBoost imitation-learning agent vs random.

Usage:
    python 17-Agent_XGBoost.py [--games N]
"""
...
def _load_play_module():
    spec = importlib.util.spec_from_file_location(
        "_play_xgb", os.path.join(os.path.dirname(os.path.abspath(__file__)), "17-Play_XGBoost.py")
    )
...
def main():
    ...
    print("Loading XGBoost agent...")
    play_mod = _load_play_module()
    xgb_agent = play_mod.XGBoostAgent()
    print(f"  from  classes : {len(xgb_agent.from_classes)}")
    print(f"  to    classes : {len(xgb_agent.to_classes)}")

    _xgb_instance = xgb_agent

    def agent_fn(obs):
        obs_dict = obs if isinstance(obs, dict) else dict(obs)
        raw = _xgb_instance.predict_actions(obs_dict)

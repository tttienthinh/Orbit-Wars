"""
Debug6: replay the game with CORRECT moves for player 0 (from pandas),
then at step 64 dump the top-per-pair tables for id_src=20 from both engines.
"""
...
def interpreter(obs, actions, step, num_agents=2):
    ...
    for f in obs0.fleets:
        ...
        for p in obs0.planets:
            if point_to_segment_distance((p[2], p[3]), op, np2) < p[4]:
                combat[p[0]].append(f); fr.append(f); hit = True; break
        if hit: continue
    ...
    def sw(planet, op, np2):
        if op == np2: return
        for f in obs0.fleets:
            if f not in fr and point_to_segment_distance((f[2], f[3]), op, np2) < planet[4]:
                combat[planet[0]].append(f); fr.append(f)
    for p in obs0.planets:
...
def build_attacks_with_angle_pd(df, player_id):
    """Build attacks_with_angle (pandas), return (attacks_with_angle, mine_across_sim)."""
    mine = (
    ...
    if mine.empty: return pd.DataFrame(), mine
    expanded = (
        mine.assign(ships_sent=(mine["ships_min"]+mine["production_src"]*NB_STEPS_SIM).apply(lambda n: list(range(1,n+1))))
        .explode("ships_sent").astype({"ships_sent":int}).reset_index(drop=True)
    )
    df_st = expanded.merge(df, how="cross").query("step > step_src and id != id_src")
    pa = (
        df_st.assign(
    ...
    if pa.empty: return pa, mine
    df_obstacles = IntervalProcessor.create_cumulative_obstacles(pa)
    awa = (
        pa.merge(df_obstacles, how="left", on=["id_src","step","ships_sent"])
...
def take_action_pd(df, player_id):
    awa, mine = build_attacks_with_angle_pd(df, player_id)
    if awa.empty: return []
    awa_comets = awa.query("nature_src == 'comet'")
    moves = []
    if not awa_comets.empty and max(max((awa_comets["x_src"]-CENTER).abs()), max((awa_comets["y_src"]-CENTER).abs())) > 45:
        ...
        id_to_avoid = awa_comets["id_src"].unique().tolist(); awa = awa.query("id_src not in @id_to_avoid")
    top5 = awa.sort_values(["step","ships_sent"],ascending=True).groupby(["id_src","id"],as_index=False).first().sort_values(["step","ships_sent"],ascending=True).groupby("id_src",as_index=False).head(5)[["id_src","id"]]
    attacks = (
        top5.merge(awa, how="left", on=["id_src","id"]).query("@player_id != owner")
...
def take_action_pd_verbose(df, player_id, target_id_src=20):
    """Return top5 selection for target_id_src."""
    awa, mine = build_attacks_with_angle_pd(df, player_id)
    if awa.empty: return None, None, None
    awa_comets = awa.query("nature_src == 'comet'")
    if not awa_comets.empty and max(max((awa_comets["x_src"]-CENTER).abs()), max((awa_comets["y_src"]-CENTER).abs())) > 45:
        id_to_avoid = awa_comets["id_src"].unique().tolist()
        awa = awa.query("id_src not in @id_to_avoid")
    awa20 = awa[awa["id_src"]==target_id_src]
    top_per_pair_20 = awa20.sort_values(["step","ships_sent"],ascending=True).groupby(["id_src","id"],as_index=False).first().sort_values(["step","ships_sent"],ascending=True)
    top5_20 = top_per_pair_20.groupby("id_src",as_index=False).head(5)
    return awa20, top_per_pair_20, top5_20
...
def build_attacks_with_angle_lz(df, player_id):
    ...
    if mine.is_empty(): return pl.DataFrame(), mine
    ...
    if pa.is_empty(): return pa, mine
    df_obstacles = IntervalProcessorPolars.create_cumulative_obstacles(pa)
    awa = (
        pa.lazy().join(df_obstacles.lazy(), on=["id_src","step","ships_sent"], how="left")
...
def take_action_lz_verbose(df, player_id, target_id_src=20):
    awa, mine = build_attacks_with_angle_lz(df, player_id)
    if awa.is_empty(): return None, None, None
    awa_comets = awa.filter(pl.col("nature_src")=="comet")
    if not awa_comets.is_empty():
        ...
        if max(x_off, y_off) > 45:
            ...
            awa = awa.filter(~pl.col("id_src").is_in(avoid))
    awa20 = awa.filter(pl.col("id_src")==target_id_src)
    top_per_pair_20 = (
        awa20.lazy().sort(["step","ships_sent"]).group_by(["id_src","id"],maintain_order=True).first().sort(["step","ships_sent"]).collect()
    )
    top5_20 = top_per_pair_20.lazy().group_by("id_src",maintain_order=True).head(5).collect()
    return awa20, top_per_pair_20, top5_20
...
for env_step in range(N_STEPS):
    ...
    moves_pd = take_action_pd(df, player_id=0)

    if env_step == 64:
        print(f"=== Step {env_step}: verbose comparison for id_src=20 ===")
        awa_pd, top_pp_pd, top5_pd = take_action_pd_verbose(df, 0, target_id_src=20)
        awa_lz, top_pp_lz, top5_lz = take_action_lz_verbose(df, 0, target_id_src=20)

        print(f"\nPD awa for id_src=20: {len(awa_pd)} rows")
        print(f"LZ awa for id_src=20: {len(awa_lz)} rows")

        print(f"\nPD top-per-pair for id_src=20 (sorted by step,ships_sent):")
        print(top_pp_pd[["id_src","id","step","ships_sent"]].to_string())

        print(f"\nLZ top-per-pair for id_src=20 (sorted by step,ships_sent):")
        print(top_pp_lz.select(["id_src","id","step","ships_sent"]).to_pandas().to_string())

        print(f"\nPD top-5 for id_src=20:")
        print(top5_pd[["id_src","id","step","ships_sent"]].to_string())

        print(f"\nLZ top-5 for id_src=20:")
        print(top5_lz.select(["id_src","id","step","ships_sent"]).to_pandas().to_string())

        # Are they identical?
        pd_ids = sorted(top5_pd["id"].tolist())
        lz_ids = sorted(top5_lz["id"].to_list())
        print(f"\nPD top-5 ids: {pd_ids}")
        print(f"LZ top-5 ids: {lz_ids}")
        print(f"Match: {pd_ids == lz_ids}")
        break
